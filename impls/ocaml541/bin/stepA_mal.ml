...
let rec qq_iter elts =
  List.fold_left
    (fun acc elt ->
      match elt with
      | List (v, _) when List.length v = 2 -> (
          match List.nth v 0 with
          | Sym s when s = "splice-unquote" ->
              mal_list [ Sym "concat"; List.nth v 1; acc ]
          | _ -> mal_list [ Sym "cons"; quasiquote elt; acc ])
      | _ -> mal_list [ Sym "cons"; quasiquote elt; acc ])
    (mal_list []) (List.rev elts)
...
and quasiquote ast =
  ...
  | List (v, _) ->
      if List.length v = 2 then
        match List.nth v 0 with
        | Sym s when s = "unquote" -> List.nth v 1
        | _ -> qq_iter v
      else qq_iter v
  | Vector (v, _) -> mal_list [ Sym "vec"; qq_iter v ]
  | Hash _ | Sym _ -> mal_list [ Sym "quote"; ast ]
  | _ -> ast
  ...
  | List (l, _) -> (
      if List.length l = 0 then Ok ast
        ...
        | Sym a0sym when a0sym = "let*" ->
            ...
            in
            eval a2 let_env
        | Sym a0sym when a0sym = "quote" -> Ok (List.nth l 1)
        | Sym a0sym when a0sym = "quasiquote" ->
            eval' (quasiquote (List.nth l 1))
        | Sym a0sym when a0sym = "defmacro!" -> (
            let a1 = List.nth l 1 in
            let a2 = List.nth l 2 in
            let* r = eval' a2 in
            match r with
            | MalFunc f -> env_set env a1 (MalFunc { f with is_macro = true })
        ...
        | Sym a0sym when a0sym = "do" ->
            ...
            eval' last
        | Sym a0sym when a0sym = "try*" -> (
            if List.length l < 3 then eval' (List.nth l 1)
            else
              match eval' (List.nth l 1) with
              | Error exc -> (
                  match List.nth l 2 with
                  | List (c, _) ->
                      let new_env = env_new (Some env) in
                      let* _ = env_set new_env (List.nth c 1) exc in
                      eval (List.nth c 2) new_env
        ...
        | _ -> (
            ...
            match f with
            | MalFunc { fn; is_macro; _ } when is_macro ->
                let* fn = fn (List.tl l) in
                eval' fn
            | Func (fn, _) | MalFunc { fn; _ } ->
...
let () =
  ...
  re {|(def! *host-language* "ocaml5.41")|} env;

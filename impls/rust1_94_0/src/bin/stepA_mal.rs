    ...
    loop {
        ...
        match &ast {
            ...
            MalObject::List(MalList { items, .. }) => match items.as_slice() {
                ...
                [MalObject::Symbol(key), body] if key.name.to_string() == "try*" => {
                    return eval(&body, &mut env);
                }
                [MalObject::Symbol(key), body, handle_catch] if key.name.to_string() == "try*" => {
                    match handle_catch {
                        MalObject::List(MalList { items, .. }) => match items.as_slice() {
                            [
                                MalObject::Symbol(MalSymbol { name, .. }),
                                MalObject::Symbol(ex_sym),
                                handler,
                            ] if name.to_string() == "catch*" => match eval(&body, &mut env) {
                                Ok(r) => return Ok(r),
                                Err(MalError::UserError(ex)) => {
                                    let mut new_env = Env::new(Some(env.clone()), None, None)?;
                                    new_env.set(ex_sym.clone(), ex.clone());
                                    return eval(&handler, &mut new_env);
                                }
                                Err(err) => {
                                    let err_msg: String = match err {
                                        MalError::UserError(_) => "user-error".to_string(),
                                        MalError::ParseError(s) => s,
                                        MalError::InvalidArguments => {
                                            "invalid-arguments".to_string()
                                        }
                                        MalError::RuntimeError(s) => s,
                                        MalError::NotFound(s) => s,
                                    };
                                    let err_msg_obj = MalObject::String(MalString::new(&err_msg));
                                    let mut new_env = Env::new(Some(env.clone()), None, None)?;
                                    new_env.set(ex_sym.clone(), err_msg_obj);
                                    return eval(&handler, &mut new_env);
                                }
                            },
                            _ => {
                                return Err(MalError::RuntimeError(
                                    "Invalid catch* structure".to_string(),
                                ));
                            }
                        },
                        _ => {
                            return Err(MalError::RuntimeError(
                                "Invalid try* structure".to_string(),
                            ));
                        }
                    }
                }
...
fn main() -> Result<(), MalError> {
    ...
    env.set(
        MalSymbol::new("*host-language*"),
        MalObject::String(MalString::new("rust")),
    );
    env.set(
        MalSymbol::new("eval"),

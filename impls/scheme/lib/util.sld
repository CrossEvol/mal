...
(define (list->alist items . rest)
  (let ((test (if (null? rest) eqv? (car rest))))
    (let loop ((items items)
               (acc '()))
      (if (null? items)
          (reverse acc)
          (let ((key (car items)))
            ...
            (if (containsKey? acc test key)
                (let ((value (cadr items))
                      (acc (dissoc acc test key)))
                  (loop (cddr items)
...
(define (containsKey? entries test key)
  (let loop ((entries entries))
    (if (pair? entries)
        (if (test (caar entries) key)
            #t
            (loop (cdr entries)))
        #f)))

(define (dissoc alist test key)
    ...
    (if (pair? entries)
        (let* ((entry (car entries))
          ...
          (if (test k key)
              ...
              (loop (cdr entries) (cons entry acc))))
        (reverse acc))))
